...
#       origin = (0,0)
        for Line in RoomDef.readlines() :
                ...
                if Line == "" :
                        ...
                        size = (int(Tokens[1].strip())+2, int(Tokens[2].strip())+2)
                elif Keyword == "Obstacle" : # Obstacle (x, y, width, height)
                        obstacles.append((      1 + int(Tokens[1].strip()), \
                                                                1 + int(Tokens[2].strip()), \
                                                                1 + int(Tokens[1].strip()) + int(Tokens[3].strip()) - 1, \
                                                                1 + int(Tokens[2].strip()) + int(Tokens[4].strip()) - 1))
...
#       print "Robot starts at : " + str(origin)
        ...
        draw.rectangle((1, 1, size[0]-1 -1, size[1]-1 -1), fill = 255);
