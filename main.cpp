...
player_pose2d locateCan(const cv::Mat &imgClean) {
	...
	cv::findContours(maskCoca, contoursCoca, CV_RETR_EXTERNAL,
			CV_CHAIN_APPROX_NONE);
	cv::vector<cv::vector<cv::Point> > contoursSprite;
	cv::findContours(maskSprite, contoursSprite, CV_RETR_EXTERNAL,
			CV_CHAIN_APPROX_NONE);
	...
	if (bestArea != -1) {
		...
		if (ellipseBox.center.y + ellipseBox.size.height / 2 > cam_height
			...
			if (VERBOSITY & 2)
				printf("Reached the %s can, try to grab it.\n",
						bestCoca ? "Coke" : "Sprite");
		}
	}
...
void init() {
	...
	for (int i = 0; i < mapSize[0]; i++) {
		for (int j = 0; j < mapSize[1]; j++) {
			wanderField[(i * mapSize[1] + j) * 2] = ((double) rand())
					/ ((double) RAND_MAX);
			wanderField[(i * mapSize[1] + j) * 2 + 1] = ((double) rand())
					/ ((double) RAND_MAX);
		}
	}
